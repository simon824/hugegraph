...
import static org.apache.hugegraph.example.ExampleUtil.registerPlugins;
...
public class Example1 {
    ...
    private static void testComputerLoad() {
        registerPlugins();
        Map<String, Object> configs = new HashMap<>();
        String pdPeers = "10.157.12.66:8686";
        ...
        propConfig.setProperty("graphspace",
                               "DEFAULT");
        propConfig.setProperty("store",
                               "hugegraph");
        ...
        hugeGraph.graphSpace("DEFAULT");

        PDClient pdClient = PDClient.create(
                PDConfig.of("10.157.12.66:8686").setEnableCache(true));
        String graph = "DEFAULT/hugegraph/g";
        List<Metapb.Partition> partitions = null;
        try {
            partitions = pdClient.getPartitions(0, graph);
        } catch (Exception e) {
            e.printStackTrace();
        }
        for (Metapb.Partition partition : partitions) {
            ...
            query.scan(String.valueOf(partition.getStartKey()),
                       String.valueOf(partition.getEndKey()));

            Iterator<Vertex> vertices = hugeGraph.vertices(query);
            while (vertices.hasNext()) {
                System.out.println(vertices.next());
            }
        }

        hugeGraph.readMode(GraphReadMode.ALL);
        Iterator<Vertex> vertices = hugeGraph.traversal().V();
        while (vertices.hasNext()) {
            // vertices.next();
            System.out.println(vertices.next());
        }
    ...
    private static void pdMetaGet(String key) {
        // String graphSpace="HUGEGRAPH/hg/GRAPHSPACE/CONF/DEFAULT";
        PDConfig pdConfig = PDConfig.of("127.0.0.1:8686")
                                    .setEnableCache(true);
        try (KvClient c = new KvClient(pdConfig)) {
            KResponse kResponse = c.get(key);
            Pdpb.Error error = kResponse.getHeader().getError();
            Map map = JSON.parseObject(kResponse.getValue(), HashMap.class);
            System.out.println(map.get("storage_limit"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private static void pdMetaSet(String key) {
        // String graphSpace="HUGEGRAPH/hg/GRAPHSPACE/CONF/DEFAULT";
        PDConfig pdConfig = PDConfig.of("127.0.0.1:8686")
                                    .setEnableCache(true);
        try (KvClient c = new KvClient(pdConfig)) {
            Map map = new HashMap();
            map.put("storage_limit", 1);
            KvResponse response = c.put(key, JSONObject.toJSONString(map));
            Pdpb.Error error1 = response.getHeader().getError();
    ...
    private static void pdMetaScan(String prefix) {
        PDConfig pdConfig = PDConfig.of("127.0.0.1:8686")
                                    .setEnableCache(true);
        try (KvClient c = new KvClient(pdConfig)) {
            Map map = new HashMap();
            map.put("storage_limit", 1);
            ScanPrefixResponse response = c.scanPrefix(prefix);
            Pdpb.Error error1 = response.getHeader().getError();
            Map<String, String> kvs = response.getKvsMap();
    ...
    private static void testComputerReadByPartitions() {
        // Pd address
        String pdPeers = "10.157.12.66:8386";

        // Get all partitions
        PDClient pdClient = PDClient.create(
                PDConfig.of(pdPeers).setEnableCache(true));
        String graph = "DEFAULT/hlm/g+v";
        List<Metapb.Partition> partitions = null;
        try {
            partitions = pdClient.getPartitions(0, graph);
        } catch (Exception e) {
        ...
        configs.put("gremlin.graph", "org.apache.hugegraph.HugeFactory");
        Configuration propConfig = new MapConfiguration(configs);
        propConfig.setProperty("graphspace", "DEFAULT");
        propConfig.setProperty("store", "hugegraph");
        HugeConfig hugeConfig = new HugeConfig(propConfig);
        // Construct graph
        HugeGraph hugeGraph;
        try {
        ...
        hugeGraph.graphSpace("DEFAULT");
        hugeGraph.readMode(GraphReadMode.ALL);
        MetaManager.instance().connect("hg_wen",
                                       MetaManager.MetaDriverType.PD,
                                       null, null, null,
                                       ImmutableList.of(pdPeers));
        // Read all vertices by partitions
        for (Metapb.Partition partition : partitions) {
            ConditionQuery query = new ConditionQuery(HugeType.VERTEX);
            query.capacity(Query.NO_CAPACITY);
            query.scan(String.valueOf(partition.getStartKey()),
    ...
    private static void testScanTable(String graphspace,
                                      String graph,
                                      String table) {
        /*
         * Valid table is:
        ...
        HgStoreClient storeClient = HgStoreClient.create(
                PDConfig.of("10.157.12.66:8386").setEnableCache(true));
        String storeTemplate = "%s/%s/g";
        String store = String.format(storeTemplate, graphspace, graph);
    ...
    private static void testComputerWriteAndRead() {
        // Register plug-ins
        registerPlugins();
        ...
        hugeGraph.schema().propertyKey("olap_page_rank").asDouble()
                 .writeType(WriteType.OLAP_RANGE).create();
        hugeGraph.traversal().V().next();
        hugeGraph.addVertex(T.label, "男人", "姓名", "家兰兰2", "性别", "男", "年龄", 0,
                            "特点", "xxxx", "亲疏", "b-l4");
        hugeGraph.tx().commit();
        GraphTransaction gtx = Whitebox.invoke(hugeGraph.getClass(),
                                               "graphTransaction", hugeGraph);
        HugeVertex olapV = HugeVertex.create(gtx, IdGenerator.of("1:斯瓦辛格"),
                                             VertexLabel.OLAP_VL);
        olapV.property("olap_page_rank", 0.3333333333D);
        gtx.addVertex(olapV);
        hugeGraph.tx().commit();
        Vertex v = hugeGraph.traversal().V("1:家兰兰2").next();
        ...
        Vertex vertex = hugeGraph.traversal().V("1:贾政").next();
        System.out.println(String.valueOf(vertex.value("olap_page_rank")));
    ...
    public static void loadSchema(final HugeGraph graph) {
        ...
        // schemaManager.getVertexLabel("ingredient").index("byIngredient").materialized().by
        // ("name").add();
        // schemaManager.getVertexLabel("reviewer").index("byReviewer").materialized().by("name")
        // .add();
    ...
    public static void testQuery(final HugeGraph graph) {
        ...
        vertexList = vertices.toList();
        assert vertexList.size() == 1;
        System.out.printf(">>>> query all authors lived %s: %s%n",
                          addr, vertexList);
    }
