...
public class IndexLabelAPI extends API {
    ...
    public Map<String, Id> delete(@Context GraphManager manager,
        ...
        Id task = g.schema().indexLabel(name).remove();
    ...
    @Consumes(APPLICATION_JSON)
    @Produces(APPLICATION_JSON_WITH_CHARSET)
    public String manage(@Context GraphManager manager,
                         @PathParam("graphspace") String graphSpace,
                         @PathParam("graph") String graph,
                         @PathParam("name") String name,
                         Map<String, String> manageData) {
        LOG.debug("Graph [{}] manage index label: {} with data: {}",
                  graph, name, manageData);

        HugeGraph g = graph(manager, graphSpace, graph);
        IndexLabel indexLabel = g.indexLabel(name);
        for (Map.Entry<String, String> entry : manageData.entrySet()) {
            switch (entry.getKey()) {
                case "status":
                    indexLabel.status(SchemaStatus.valueOf(entry.getValue()));
                    break;
                default:
                    throw new AssertionError(String.format(
                            "Not support manage %s now", entry.getKey()));
            }
        }
        g.updateIndexLabel(indexLabel);
        return manager.serializer().writeIndexlabel(indexLabel);
    }
