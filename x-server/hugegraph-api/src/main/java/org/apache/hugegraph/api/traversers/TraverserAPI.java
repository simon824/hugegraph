...
public class TraverserAPI extends API {
    ...
    protected static Steps steps(HugeGraph graph, Step step) {
        Map<String, Map<String, Object>> eSteps = new HashMap<>();
        if (step != null && step.properties != null) {
            for (Map.Entry<String, Object> entry : step.properties.entrySet()) {
                eSteps.put(entry.getKey(), (Map<String, Object>) entry.getValue());
            }
        }
        return new Steps(graph, step.direction, eSteps, null,
                         step.maxDegree, step.skipDegree);
    }
