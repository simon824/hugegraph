...
public class ConditionTest extends BaseUnitTest {
    ...
    public void testConditionNot() {
        Condition c1 = Condition.lt(HugeKeys.ID, 18);
        Condition c2 = c1.not();
        Condition c3 = Condition.eq(HugeKeys.ID, "123").not();

        Assert.assertEquals(c2, c2.copy());
        Assert.assertEquals(c2.hashCode(), c2.hashCode());

        Assert.assertFalse(c1.test(29));
        Assert.assertTrue(c2.test(29));
        Assert.assertTrue(c1.test(17));
        Assert.assertFalse(c2.test(17));

        Assert.assertEquals("NOT ID == 123", c3.toString());
        Assert.assertFalse(c3.isRelation());
        Assert.assertTrue(c3.isSysprop());
        Assert.assertFalse(c3.isFlattened());
        Assert.assertTrue(c3.isLogic());
