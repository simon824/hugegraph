...
import org.apache.hugegraph.testutil.FakeObjects.FakeVertex;
...
public class VertexCoreTest extends BaseCoreTest {
    ...
    public void testAddVertexWithCollectionIndex() {
        ...
        vertices = graph.traversal().V()
                        .has("soft", "category",
                             ConditionP.textContains("database"))
                        .toList();
    ...
    public void testAddVertexWithCustomizeStringIdStrategyWithoutValidId() {
        ...
        // Expect id length <= 16384
        String id = new String(new byte[16383]) + ".";
        assert id.length() == 16384;
        ...
        // Expect id length <= 16384
        Assert.assertThrows(IllegalArgumentException.class, () -> {
            String largeId = new String(new byte[16384]) + ".";
            assert largeId.length() == 16385;
    ...
    public void testQueryByInvalidSysprop() {
        ...
        PropertyKey namePK = graph.schema().getPropertyKey("name");
    ...
    public void testQueryByDateProperty() {
        ...
        Assert.assertTrue(dates[1].equals(vertices.get(0).value("birth"))
                          || dates[2].equals(vertices.get(0).value("birth"))
                          || dates[3].equals(vertices.get(0).value("birth")));
    }
    ...
    public void testQueryByTextContainsPropertyOrderByMatchedCount() {
        ...
        Assert.assertEquals("James", vertices.get(2).value("name"));
        assertContains(vertices,
        ...
        Assert.assertEquals("James", vertices.get(1).value("name"));
    ...
    public void testQueryByJointIndexesWithSearchAndRangeIndexes() {
        ...
        vertices = graph().traversal().V()
                          .has("age", P.gte(2))
                          .has("description", Text.contains("yellow"))
                          .toList();
        Assert.assertEquals(2, vertices.size());
        ...
        vertices = graph().traversal().V()
                          .has("age", P.gt(0))
                          .has("description", Text.contains("black"))
                          .toList();
        Assert.assertEquals(1, vertices.size());
    }
    ...
    public void testQueryByJointIndexesWithTwoSearchAndOneRangeIndexes() {
        ...
        vertices = graph().traversal().V()
                          .has("age", P.gt(0))
                          .has("description", Text.contains("golden"))
    ...
    public void testQueryByUniqueIndex() {
        ...
        // 测试使用unique索引查询数据
        PropertyKey pk = schema.getPropertyKey("name");
        VertexLabel el = schema.getVertexLabel("node");
        ConditionQuery cq = new ConditionQuery(HugeType.VERTEX);
        cq.eq(HugeKeys.LABEL, el.id());
        cq.query(Condition.eq(pk.id(), "tom"));
        graph.vertices(cq).next();
        Assert.assertEquals(cq.optimized(), ConditionQuery.OptimizedType.INDEX);
    }

    @Test
    public void testQueryByOneIndex() {
        HugeGraph graph = this.graph();
        SchemaManager schema = graph().schema();
        schema.vertexLabel("user")
              .properties("name", "city", "age")
        ...
        PropertyKey namepk = schema.getPropertyKey("name");
        PropertyKey agepk = schema.getPropertyKey("age");
        VertexLabel el = schema.getVertexLabel("user");
        ConditionQuery cq = new ConditionQuery(HugeType.VERTEX);
        cq.eq(HugeKeys.LABEL, el.id());
        cq.query(Condition.eq(namepk.id(), "Tom"));
        cq.query(Condition.eq(agepk.id(), 19));

        Iterator<Vertex> iters = graph.vertices(cq);

        Assert.assertEquals(cq.optimized(),
                            ConditionQuery.OptimizedType.INDEX);
        Assert.assertEquals(1, Iterators.size(iters));
    }
    ...
    public void testQueryBySingleSearchPropertyInPage() {
        ...
        iter = g.V().has("city",
                         Text.containsAny(ImmutableSet.of("Beijing", "Shanghai")))
                .has("~page", "").limit(1);
        Assert.assertEquals(1, IteratorUtils.count(iter));
        ...
        vertices = g.V().has("city",
                             Text.containsAny(ImmutableSet.of("Beijing",
                                                              "Shanghai")))
                    .has("~page", page).limit(20).toList();
        Assert.assertEquals(17, vertices.size());
        Assert.assertTrue(vertices.contains(vertex3));
    ...
    public void testQueryByJointPropertyInPage() {
        ...
        Assert.assertThrows(HugeException.class, () -> {
            g.V().hasLabel("programmer").has("name", "marko")
             .has("city", Text.contains("Beijing"))
             .has("~page", "").limit(10).toList();
        });

        Assert.assertThrows(HugeException.class, () -> {
            g.V().hasLabel("programmer").has("age", 30).has("city", Text.contains("Beijing"))
             .has("~page", "").limit(10).toList();
        });
    ...
    public void testQueryVerticesByLabelsWithOneLabelNotExist() {
        ...
        Assert.assertThrows(IllegalArgumentException.class, () -> {
            ...
            Assert.assertContains("Undefined vertex label: 'user3'",
                                  e.getMessage());
        });
        ...
        Assert.assertThrows(IllegalArgumentException.class, () -> {
            ...
            Assert.assertContains("Undefined vertex label: 'user4'",
                                  e.getMessage());
        });
    }
