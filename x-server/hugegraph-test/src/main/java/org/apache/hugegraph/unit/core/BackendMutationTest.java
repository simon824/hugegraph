...
public class BackendMutationTest extends BaseUnitTest {
    ...
    private static BackendEntry constructBackendEntry(String id,
        ...
        Id id1 = IdGenerator.of(id);
        BytesBuffer buffer = BytesBuffer.allocate(1 + id1.asBytes().length);
        BinaryBackendEntry entry = new BinaryBackendEntry(HugeType.VERTEX,
                                                          buffer.writeId(id1).bytes());
        if (columns.length == 2) {
            String subId = SplicingIdGenerator.concat(id, columns[0]);
            entry.subId(IdGenerator.of(subId));
        }
        for (int i = 0; i < columns.length; i = i + 2) {
            entry.column(BackendEntry.BackendColumn.of(columns[i].getBytes(),
                                                       columns[i + 1].getBytes()));
        }
        return entry;
    }
    ...
    private static List<BackendAction> get(BackendMutation mutation, String id) {
        Id id1 = IdGenerator.of(id);
        BytesBuffer buffer = BytesBuffer.allocate(1 + id1.asBytes().length);
        BinaryBackendEntry.BinaryId id2 =
                new BinaryBackendEntry.BinaryId(buffer.writeId(id1).bytes(), id1);
        return mutation.mutation(HugeType.VERTEX, id2);
    }
    ...
    public void testInsertAndInsertEntry() {
        ...
        mutation.addWithOptimize(entry1, Action.INSERT);
        mutation.addWithOptimize(entry2, Action.INSERT);
        mutation.addWithOptimize(entry3, Action.INSERT);
    ...
    public void testInsertAndDeleteEntry() {
        ...
        Assert.assertThrows(HugeException.class, () -> {
            mutation.addWithOptimize(entry2, Action.DELETE);
        });
    }
    ...
    public void testInsertAndAppendEntry() {
        ...
        mutation.addWithOptimize(entry1, Action.INSERT);
        Assert.assertThrows(HugeException.class, () -> {
            mutation.addWithOptimize(entry2, Action.APPEND);
        });
    }
    ...
    public void testInsertAndEliminateEntry() {
        ...
        mutation.addWithOptimize(entry1, Action.INSERT);
        Assert.assertThrows(HugeException.class, () -> {
            mutation.addWithOptimize(entry2, Action.ELIMINATE);
        });
    }
    ...
    public void testDeleteAndInsertEntry() {
        ...
        mutation.addWithOptimize(entry1, Action.DELETE);
        mutation.addWithOptimize(entry2, Action.DELETE);
        mutation.addWithOptimize(entry3, Action.INSERT);
    ...
    public void testAppendAndInsertEntry() {
        ...
        mutation.addWithOptimize(entry1, Action.APPEND);
        mutation.addWithOptimize(entry2, Action.INSERT);
    ...
    public void testEliminateAndInsertEntry() {
        ...
        mutation.addWithOptimize(entry1, Action.ELIMINATE);
        mutation.addWithOptimize(entry2, Action.INSERT);
