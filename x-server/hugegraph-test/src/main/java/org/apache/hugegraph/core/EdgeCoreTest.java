...
import org.apache.hugegraph.testutil.FakeObjects.FakeEdge;
...
public class EdgeCoreTest extends BaseCoreTest {
    ...
    public void testAddEdgeWithOverrideEdge() {
        ...
        Vertex person2 = graph.addVertex(T.label, "person",
                                         "name", "person2", "age", 24,
                                         "city", "Shanghai");
        person1.addEdge("trade", person2,
                        "trade_money", 100, "time", "2023-3-28");
        person1.addEdge("trade", person2,
                        "trade_money", 90, "time", "2023-3-28");
        graph.tx().commit();
        edges = graph.traversal().E().toList();
        Assert.assertEquals(6, edges.size());
        Assert.assertEquals(90, graph.traversal().V(person1.id())
                                     .outE("trade").toList().get(0).property("trade_money")
                                     .value());
    }
    ...
    @Test
    public void testAddGeneralEdgeWithSouceLabelAndTargetLabel() {
        HugeGraph graph = graph();
        Assert.assertThrows(IllegalArgumentException.class, () -> {
            graph.schema().edgeLabel("test").asGeneral().sourceLabel("person").targetLabel(
                    "person").properties("name").create();
        });
    }
    ...
    public void testAddEdgeWithLargeSortkey() {
        ...
        Assert.assertThrows(IllegalArgumentException.class, () -> {
            ...
            Assert.assertContains("The max length of edge id is 1048576",
                                  e.getMessage());
        });
    }

    // TODO:for cheng:因单query和照军的多query接口实现不同，这里不一定抛出异常，临时关闭
    // @Test
...
//        Assert.assertEquals("~undefined", adjacent.label());
        ...
        Whitebox.setInternalState(params().graph(),
                                  "checkAdjacentVertexExist", true);
        try {
            /**
             * 1、由于在 HugeVertexStep edges() 中查邻接边接口 替换成了批量接口 graph.edges
             * (Iterator<Query>)
             * 2、之前 graph.edges(Query)
             * 在CachedGraphTransaction中有实现，所以在本次查询可以命中Cache. 上面通过执行adjacent
             * .forceLoad()将adjacent.label置为了undefined, 所以可以抛出vertex not
             * exist exception.
             * 3、替换之后,在CachedGraphTransaction新接口无法实现,没有Cache, 每次执行都是从底层查询,
             * 故adjacent.label不是undefined,未抛出exception
             */
            //Assert.assertThrows(HugeException.class, () -> {
            //    // read from cache
            //    try {
            ...
            Assert.assertEquals(0,
                                graph.traversal().V(james.id()).outE().
                                     has("score", 3).otherV().
                                     values().toList().size());
        } finally {
            Whitebox.setInternalState(params().graph(),
...
//            Assert.assertEquals("~undefined", adjacent.label());
    ...
    public void testQueryByOutEWithDateProperty() {
        ...
        List<Edge> edges = graph.traversal().V().outE()
                                ...
                                .toList();
        Assert.assertEquals(2, edges.size());
        Assert.assertEquals(ImmutableSet.of(Utils.date(dates[0]),
                                            Utils.date(dates[1])),
                            ImmutableSet.of(edges.get(0).value("date"),
                                            edges.get(1).value("date")));
    }
    ...
    public void testScanEdgeInPaging() {
        ...
        // 这里只能通过HugeType.VERTEX 获取shard信息, HugeType.EDGE获取不到
        List<Shard> shards = graph.metadata(HugeType.VERTEX, "splits", 67108864L);
    ...
    public void testQueryBothEdgesOfVertexInPaging() {
        ...
        Vertex person1 = graph.addVertex(T.label, "person",
                                         "name", "person1", "age", 25,
                                         "city", "Beijing");
        james.addEdge("trade", person1, "time", "2023-03-29", "trade_money", 90);
        graph.tx().commit();

        count = 0;
        page = PageInfo.PAGE_NONE;
        while (page != null) {
            GraphTraversal<?, ?> iterator = graph.traversal().V(james).bothE()
                                                 .has("~page", page).limit(1);
            long size = IteratorUtils.count(iterator);
    ...
    @Test
    public void testQueryByConditionSink() {

        HugeGraph graph = graph();

        init18Edges();

        Assert.assertEquals(1L, graph.traversal().V().hasLabel("author").where(
                __.outE().has("score", P.within(3))).count().next());
    }
    ...
    public void testRemoveEdgeOneByOne() {
        ...
        // General类型的测试
        initGeneralSchema();
        guido.addEdge("trade", java2, "time", "2023-03-29", "trade_money", 90);
        edges = graph.traversal().E().toList();
        Assert.assertEquals(6, edges.size());

        for (int i = 0; i < edges.size(); i++) {
            edges.get(i).remove();
            Assert.assertEquals(5 - i, graph.traversal().E().toList().size());
        }
    }
    ...
    public void testRemoveEdgesOfSuperVertex() {
        ...
        // 添加General类型的边
        initGeneralSchema();
        guido.addEdge("trade", java1, "time", "2023-03-29", "trade_money", 90);
        guido.addEdge("trade", java2, "time", "2023-03-29", "trade_money", 90);
        edges = graph.traversal().E().toList();
        Assert.assertEquals(txCap + 5 + 2, edges.size());
    ...
    public void testRemoveEdgePropertyNullableWithIndex() {
        ...
        // General 类型的测试
        initGeneralSchema();
        Edge edge1 = louise.addEdge("trade", sean,
                                    "trade_money", 90, "time", "2023-03-30");
        edge1.property("trade_money").remove();
        graph.tx().commit();
    }
    ...
    public void testGeneralEdges() {
        ...
        Vertex baidu = graph.addVertex(T.label, "company",
                                       "name", "Baidu", "city", "Beijing");

        Vertex huawei = graph.addVertex(T.label, "company",
                                        "name", "Huawei", "city", "Shenzhen");
        ...
        person1.addEdge("trade", baidu,
                        "trade_money", 100, "time", "2023-3-28");

        baidu.addEdge("trade", huawei,
                      "trade_money", 100, "time", "2023-3-28");

        baidu.addEdge("trade", person1,
                      "trade_money", 100, "time", "2023-3-28");

        graph.tx().commit();

        List<Vertex> vertex = graph.traversal().V().toList();
        List<Edge> edges = graph.traversal().E().toList();
        Assert.assertEquals(4, edges.size());

        List<Vertex> vertices =
                graph.traversal().V(person1.id()).out().toList();
        Assert.assertEquals(2, vertices.size());

        edges = graph.traversal().V(person1.id()).outE().toList();
        Assert.assertEquals(2, edges.size());

        edges = graph.traversal().E().hasLabel("trade").toList();
        Assert.assertEquals(4, edges.size());

        vertices = graph.traversal().V(baidu.id()).out("trade").toList();
        Assert.assertEquals(2, vertices.size());
    }

    @Test
    public void testGeneralEdgesWithSecondaryAndRangeIndex() {
        HugeGraph graph = graph();
        SchemaManager schema = graph.schema();
        initGeneralSchema();

        LOG.debug("===============   create indexLabel  ================");
        schema.indexLabel("tradeByTime").onE("trade").secondary().by("time")
              .create();
        schema.indexLabel("tradeByMoney").onE("trade").range().by("trade_money")
              .create();
        ...
        Vertex baidu = graph.addVertex(T.label, "company", "name", "Baidu", "city", "Beijing");
        Vertex huawei = graph.addVertex(T.label, "company", "name", "Huawei", "city", "Shenzhen");
        person1.addEdge("trade", person2, "trade_money", 100, "time", "2023-3-28");
        person1.addEdge("trade", baidu, "trade_money", 90, "time", "2023-3-27");
        baidu.addEdge("trade", huawei, "trade_money", 80, "time", "2023-3-26");
        baidu.addEdge("trade", person1, "trade_money", 70, "time", "2023-3-25");
        graph.tx().commit();

        List<Edge> edges =
                graph.traversal().E().has("time", "2023-3-28").toList();
        Assert.assertEquals(1, edges.size());
        Assert.assertEquals("2023-3-28", edges.get(0).value("time"));

        edges =
                graph.traversal().E().has("trade_money", P.gt(80)).toList();
        Assert.assertEquals(2, edges.size());
    }
    ...
    private void init4GeneralEdges() {
        HugeGraph graph = graph();
        initGeneralSchema();
        ...
        baidu.addEdge("trade", huawei,
                      "trade_money", 80, "time", "2023-3-28");

        baidu.addEdge("trade", person1,
                      "trade_money", 70, "time", "2023-3-28");

        graph.tx().commit();
    }

    private void initGeneralSchema() {
        SchemaManager schema = graph().schema();
        // propertyKeys
        schema.propertyKey("trade_money").asInt().create();
        // vertexLabels
        schema.vertexLabel("company")
              ...
              .create();
        // edgeLabels
        schema.edgeLabel("trade")
              .asGeneral()
              .properties("trade_money", "time")
              .multiTimes().sortKeys("time")
              .nullableKeys("trade_money")
              .enableLabelIndex(false)
              .create();
    }
    ...
    /**
     * 该测试仅仅是为了执行CachedBackendStore.query通过单测,无其他用途
     */
    @Test
    public void testQuery() {
        HugeGraph graph = graph();
        HstoreStore hstoreStore =
                new HstoreStore.HstoreGraphStore(graph.storeProvider(),
                                                 graph.graphSpace(),
                                                 graph.backend());
        CachedBackendStore
                cachedBackendStore = new CachedBackendStore(hstoreStore);
        ConditionQuery query = new ConditionQuery(HugeType.EDGE);
        query.limit(1L);
        List<Query> queryList = Lists.newArrayList();
        queryList.add(query);
        Iterator<Iterator<BackendEntry>> result =
                cachedBackendStore.query(queryList.iterator(), t -> t, graph);
