...
public class GremlinApiTest extends BaseApiTest {
    ...
    public void testGet() {
        Map<String, Object> params =
                ImmutableMap.of("gremlin",
                                "gremlin.traversal().V().limit(1)");
        assertResponseStatus(200, client().get(path, params));
    }
    ...
    public void testScript() {
        String bodyTemplate = "{"
                              ...
                              + "\"bindings\":{},"
                              + "\"language\":\"gremlin-groovy\","
                              + "\"aliases\":{\"graph\":\"DEFAULT-gremlin\","
                              + "\"g\":\"__g_DEFAULT-gremlin\"}}";
        ...
        String script = "schema=graph.schema();"
                        ...
                        +
                        "marko=graph.addVertex(T.id,'835e1153928149578691cf79258e90eb',T.label," +
                        "'person','name','marko','age',29,'city'," +
                        "'135e1153928149578691cf79258e90eb');"
                        +
                        "vadas=graph.addVertex(T.id,'935e1153928149578691cf79258e90eb',T.label," +
                        "'person','name','vadas','age',27,'city'," +
                        "'235e1153928149578691cf79258e90eb');"
    ...
    public void testSetVertexProperty() {
        String pkPath = "/graphs/gremlin/schema/propertykeys/";
        // Cardinality single
        String foo = "{"
                     + "\"name\": \"foo\","
                     + "\"data_type\": \"TEXT\","
        ...
        String vlPath = "/graphs/gremlin/schema/vertexlabels/";
        String vertexLabel = "{"
                             + "\"name\": \"person\","
    ...
    public void testTree() {
        // NOTE: supply unmatch cardinality, but we give up the check
        String body = "{\"gremlin\":\"g.V().limit(1).both().tree()\","
                      + "\"bindings\":{},"
                      + "\"language\":\"gremlin-groovy\","
                      + "\"aliases\":{\"g\":\"__g_DEFAULT-gremlin\"}}";
        assertResponseStatus(200, client().post(path, body));
