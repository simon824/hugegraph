...
public class StandardTaskScheduler extends TaskScheduler {
    ...
    private <V> Future<?> restore(HugeTask<V> task) {
        ...
        if (this.tasks.containsKey(task.id())) {
            LOG.warn(String.format("Task '%s' is already in the queue",
                                   task.id()));
            return null;
        }
        ...
        if (task.isDone() || task.completed()) {
            ...
            LOG.warn(String.format("No need to restore completed task '%s' " +
                                   "with status %s", task.id(), task.status()));
            return null;
        }
    ...
    private <V> Future<?> submitTask(HugeTask<V> task) {
        long size = this.tasks.values().stream()
                              ...
                              .count() + 1;
        if (size > MAX_PENDING_TASKS) {
            LOG.warn(String.format("Pending tasks size %s has exceeded the " +
                                   "max limit %s", size, MAX_PENDING_TASKS));
        }
        ...
        if (this.graph.mode().loading()) {
            return this.backupForLoadTaskExecutor.submit(
                    new TaskManager.ContextRunnable(task, task.context())
            );
        }
