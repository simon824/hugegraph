...
public class RolePermission {
    ...
    public static final RolePermission NONE = RolePermission.role(ALL, ALL, HugePermission.NONE);
    public static final RolePermission ADMIN = RolePermission.role(ALL, ALL, HugePermission.ADMIN);
    public static final String ANY_LABEL = "*";
    public static final String POUND_SEPARATOR = "#";
    protected static final Logger LOG = Log.logger(RolePermission.class);
    ...
    public static RolePermission fromJson(Object json) {
        ...
        if (json instanceof String) {
            ...
            E.checkArgument(json instanceof RolePermission, "Invalid role value: %s", json);
    ...
    public boolean isSpace(String graphSpace) {
        if (graphSpace == null || this.isAdmin()) {
            return true;
        }

        return this.roles.containsKey(graphSpace) &&
               this.roles.get(graphSpace).containsKey(ALL) &&
               this.roles.get(graphSpace).get(ALL)
                         .containsKey(HugePermission.SPACE);
    }

    public boolean isAnalyst(String graphSpace) {
        if (this.isSpace(graphSpace)) {
            return true;
        }
        ...
        boolean check = true;
        Map<HugePermission, Map<String, List<HugeResource>>> accesses =
                this.roles.get(graphSpace).get(ALL);
        for (HugePermission perm : permList) {
            ...
            check = accesses.get(perm).containsKey("ALL");
        }
        return check;
    }
