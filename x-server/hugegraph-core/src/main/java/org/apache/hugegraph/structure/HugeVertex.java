...
public class HugeVertex extends HugeElement implements Vertex, Cloneable {
    ...
    public HugeType type() {
        // 对Vertex类型，当label为task时,返回TASK类型,便于根据类型获取存储表信息
        if (label != null &&
    ...
    public VertexLabel schemaLabel() {
        assert this.graph() == null || VertexLabel.OLAP_VL.equals(this.label) ||
               this.graph().sameAs(this.label.graph());
        // 如果VertexLabel是一个General类型, 根据顶点ID进行访问后更正顶点的vertexLabel
        if (this.label.generalVl()) {
    ...
    public void correctVertexLabel(VertexLabel correctLabel) {
        ...
        if (this.label != null && !this.label.undefined() &&
            !correctLabel.undefined() && !this.label.generalVl() && !correctLabel.generalVl()) {
            E.checkArgument(this.label.equals(correctLabel),
                            "[%s]'s Vertex label can't be changed from '%s' " +
                            "to '%s'", this.id, this.label, correctLabel);
        }
    ...
    public List<HugeEdge> getEdgesList() {
        return ((List<HugeEdge>) this.edges);
    }
    ...
    public HugeEdge constructEdge(String label, HugeVertex vertex,
        ...
        // Check link
        E.checkArgument(!edgeLabel.isFather(), "Adding an edge of parenttype " +
                                               "is not allowed");
        if (!edgeLabel.edgeLabelType().general()) {
            // general()类型的不用进行links的检查
    ...
    public void addOutEdge(HugeEdge edge) {
        Vertex ownerVertex = edge.ownerVertex();
        if (ownerVertex == null) {
    ...
    protected boolean ensureFilledProperties(boolean throwIfNotExist) {
        ...
        // 当schema的属性本身为空并且不是GeneralVl类型就不去查询
        if (!this.label.generalVl() &&
            this.schemaLabel().properties().isEmpty()) {
            this.propLoaded();
            return true;
        }
        ...
        if (vertex.schemaLabel().undefined() ||
            !vertex.schemaLabel().equals(this.label) &&
            !this.label.generalVl()) {
            ...
            this.correctVertexLabel(vertex.schemaLabel());
        }
