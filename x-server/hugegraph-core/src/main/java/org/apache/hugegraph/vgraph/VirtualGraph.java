...
import org.apache.hugegraph.metrics.MetricManager;
...
public class VirtualGraph {
    ...
    public VirtualGraph(HugeGraphParams graphParams) {
        ...
        this.metricsNamePrefix = MetricRegistry.name(this.getClass(),
                                                     this.graphParams.graph().graphSpace(),
                                                     this.graphParams.graph().name());
        this.init();
    }
    ...
    public void init() {
        this.vertexCache = Caffeine.newBuilder()
                                   ...
                                   .expireAfterAccess(this.graphParams.configuration().get(
                                                              CoreOptions.VIRTUAL_GRAPH_VERTEX_EXPIRE),
                                                      TimeUnit.SECONDS)
        ...
        MetricManager.INSTANCE.getRegistry().register(
                MetricRegistry.name(this.metricsNamePrefix, "vertex", "count"),
                this.vertexCountGauge);
        MetricManager.INSTANCE.getRegistry().register(
    ...
    public Iterator<HugeVertex> queryHugeVertexByIds(List<Id> vIds, VirtualVertexStatus status) {
        ...
        E.checkArgument(
                status != VirtualVertexStatus.OutEdge && status != VirtualVertexStatus.InEdge,
                "Get edges of vertex is not supported in this method, please use " +
                "queryVertexWithEdges.");
        ...
        if (missingIds.size() > 0) {
            VirtualGraphQueryTask<VirtualVertex> batchTask =
                    new VirtualGraphQueryTask<>(HugeType.VERTEX, missingIds);
    ...
    public boolean updateIfPresentVertex(HugeVertex vertex, Iterator<HugeEdge> inEdges) {
        ...
        if (vertexFromCache != null) {
            VirtualVertex vertexMerged =
                    toVirtual(vertexFromCache, vertex, vertex.getEdges(), inEdges);
    ...
    public VirtualVertex putVertex(HugeVertex vertex, Collection<HugeEdge> outEdges,
    ...
    public Iterator<HugeEdge> queryEdgeByIds(List<Id> eIds, VirtualEdgeStatus status) {
        ...
        eIds.forEach(eId -> {
            ...
            HugeVertex owner = vertexMap.computeIfAbsent(((EdgeId) eId).ownerVertexId(),
                                                         vId -> new HugeVertex(graphParams.graph(),
                                                                               vId,
        ...
        if (missingIds.size() > 0) {
            VirtualGraphQueryTask<VirtualEdge> batchTask =
                    new VirtualGraphQueryTask<>(HugeType.EDGE, missingIds);
            this.loader.add(batchTask);
            try {
    ...
    private VirtualVertex mergeVVOutEdge(VirtualVertex oldV, VirtualVertex newV,
        ...
        if (outEdges != null) {
            ...
            outEdges.forEach(e -> {
                VirtualEdge edge =
                        toVirtual(null, e);  // give up to refresh edge cache due to memory cost
                assert edge != null;
    ...
    private VirtualVertex mergeVVInEdge(VirtualVertex oldV, VirtualVertex resultV,
                                        Iterator<HugeEdge> inEdges) {
        if (inEdges != null) {
    ...
    private void listenChanges() {
        ...
        this.cacheEventListener = event -> {
            ...
            if (org.apache.hugegraph.backend.cache.Cache.ACTION_INVALID.equals(args[0])) {
                ...
                return true;
            } else if (org.apache.hugegraph.backend.cache.Cache.ACTION_CLEAR.equals(args[0])) {
    ...
    private void clearCache(HugeType type, boolean notify) {
        ...
        if (notify) {
            this.notifyChanges(org.apache.hugegraph.backend.cache.Cache.ACTION_CLEARED, null, null);
        }
    }
}