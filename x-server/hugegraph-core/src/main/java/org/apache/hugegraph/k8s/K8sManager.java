...
public class K8sManager {
    ...
    private static final Logger LOGGER = Log.logger(K8sManager.class);
    ...
    public Set<String> createOltpService(GraphSpace graphSpace, Service service,
                                         List<String> metaServers, String cluster) {

        if (null == k8sDriver) {
            LOGGER.error("startOltpService", new HugeException("k8sDriver is not initialized!"));
    ...
    @SuppressWarnings("unchecked")
    public Set<String> startOltpService(GraphSpace graphSpace, Service service,
                                        List<String> metaServers, String cluster) {
        if (null == k8sDriver) {
            LOGGER.error("startOltpService", new HugeException("k8sDriver is not initialized!"));
            return Collections.EMPTY_SET;
    ...
    public Set<String> createService(GraphSpace graphSpace, Service service,
        ...
        switch (service.type()) {
            ...
            case STORAGE:
            default:
                throw new AssertionError(String.format("Invalid service type '%s'",
                                                       service.type()));
        }
    }
    ...
    public void stopService(GraphSpace graphSpace, Service service) {
        if (null == k8sDriver) {
            LOGGER.error("stopService", new HugeException("k8sDriver is not initialized!"));
            return;
        }
        switch (service.type()) {
            case OLTP:
            ...
            case STORAGE:
            default:
                LOGGER.debug("Cannot stop service other than OLTP");
        }
    }
    ...
    public void createOperatorPod(String namespace, String imagePath) {
        if (Strings.isNullOrEmpty(imagePath)) {
            LOGGER.error("Cannot create operator pod",
                         new IllegalArgumentException("imagePath should not be empty"));
            return;
        }
    ...
    public void loadOperator(String namespace, String imagePath) throws HugeException {
        try {
            ...
            if (Strings.isNullOrEmpty(this.operatorTemplate)) {
                throw new HugeException("Cannot generate yaml config for operator: " +
            ...
            LOGGER.info("Create or replace by yaml to create operator for " +
                        "namespace {} with image {}", namespace, imagePath);
            k8sDriver.createOrReplaceByYaml(content);
        } catch (IOException e) {
            LOGGER.error("IO Exception when create operator", e);
        } catch (Exception e) {
            LOGGER.error("Unknown Exception when create operator", e);
        }
    }
    ...
    public void loadResourceQuota(String namespace, int cpuLimit, int memoryLimit) throws
        ...
        try {
            ...
            LOGGER.error("Failed to load resource quota!", e);
        } finally {
            if (null != inputStream) {
                try {
                    inputStream.close();
