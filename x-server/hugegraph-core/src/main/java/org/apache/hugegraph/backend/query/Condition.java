...
public abstract class Condition implements Serializable {
    ...
    protected static final Logger LOG = Log.logger(Condition.class);
    ...
    public enum RelationType implements BiPredicate<Object, Object> {
        ...
        TEXT_CONTAINS_ANY("textcontainsany", String.class, Collection.class,
                          (v1, v2) -> {
                              ...
                              for (String word : words) {
                                  if (((String) v1).toLowerCase().contains(word.toLowerCase())) {
                                      return true;
                                  }
                              }
                              return false;
        ...
        protected static boolean equals(final Object first,
            ...
            LOG.debug("first is: {}({}), second is: {}({})",
                      first, (first == null) ? null : first.getClass(), second,
                      second.getClass());
        ...
        protected static int compare(final Object first, final Object second) {
            ...
            } else if (second instanceof String && first instanceof String) {
...
//            super();
    ...
    public static class Not extends Condition implements Serializable {
        ...
        @Override
        public List<? extends Relation> relations() {
            List<Relation> list = new ArrayList<>(this.condition.relations());
            return list;
        }
    ...
    public static class SyspropRelation extends Relation implements Serializable {
        ...
        public boolean test(HugeElement element) {
            ...
            // 由于父子边查询,当按子边查询时, this.value为子边label id,然而element.sysprop获取的是
            // 父label id,对比为false。暂时优化成 this.value和 父 子label比较取OR
            if (element instanceof HugeEdge && this.key == HugeKeys.LABEL) {
                Object value1 = element.sysprop(HugeKeys.LABEL);
                Object value2 = element.sysprop(HugeKeys.SUB_LABEL);
                return this.relation.test(value1, this.value()) ||
                       this.relation.test(value2, this.value());
            }
            LOG.debug("element: {}, value: {}, this.key(): {} this.value(): {}",
                      element, value, this.key, this.value());
            return this.relation.test(value, this.value());
        }
