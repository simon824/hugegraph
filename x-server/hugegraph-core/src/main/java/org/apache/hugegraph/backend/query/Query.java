...
public class Query implements Cloneable {
    ...
    // isTaskQuery表示是否是任务查询(任务和点都是点存储,当开启olap查询时,任务查询也会进行olap查询,任务不准进行olap查询,
    // 故因此需要判断)--当点id和任务id有相同的时候,如果任务查询olap会报unsupported property key问题
    ...
    public long limit() {
        if (this.capacity != NO_CAPACITY) {
            E.checkArgument(this.limit == Query.NO_LIMIT ||
                            ...
                            "{\"code\":\"105-006\"," +
                            "\"attach\":[\"%s\", \"%s\"]," +
                            "\"message\":\"Invalid limit %s, " +
                            "must be <= capacity(%s)\"}",
                            this.limit, this.capacity, this.limit,
                            this.capacity);
        }
        return this.limit;
    }
    ...
    public boolean bigCapacity() {
        return this.capacity == NO_CAPACITY || this.capacity > HugeTraverser.DEF_CAPACITY;
    }
    ...
    public void setTaskQuery(boolean taskQuery) {
        isTaskQuery = taskQuery;
    }
