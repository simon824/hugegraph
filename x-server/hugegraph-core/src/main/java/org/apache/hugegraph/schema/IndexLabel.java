...
public class IndexLabel extends SchemaElement {
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            switch (entry.getKey()) {
                ...
                case P.STATUS:
                    ...
                    break;
                case P.USERDATA:
                    indexLabel.userdata((Map<String, Object>) entry.getValue());
                    break;
                case P.BASE_TYPE:
                    HugeType hugeType =
    ...
    public static void main(String[] args) {
        IndexLabel indexLabel = new IndexLabel(null, IdGenerator.of(10), "personByCity");
        indexLabel.baseType(HugeType.valueOf("VERTEX_LABEL"));
        indexLabel.baseValue(IdGenerator.of(1));
        indexLabel.indexType(IndexType.SECONDARY);
        indexLabel.indexFields(IdGenerator.of(2));

        String content = JsonUtil.toJson(indexLabel.asMap());
        System.out.println("il to json:\n" + content);

        indexLabel = IndexLabel.fromMap(JsonUtil.fromJson(content, Map.class), null);
        content = JsonUtil.toJson(indexLabel.asMap());
        System.out.println("il to json to il to json:\n" + content);
    }
