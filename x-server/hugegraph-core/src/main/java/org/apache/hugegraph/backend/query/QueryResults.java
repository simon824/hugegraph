...
public class QueryResults<R> {
    ...
    private static final CIter<?> EMPTY_ITERATOR = new EmptyIterator<>();
    ...
    public static <T> ListIterator<T> toList(Iterator<T> iterator) {
        try {
            return new ListIterator<>(HugeTraverser.DEF_CAPACITY, iterator);
        } finally {
    ...
    public static <T, R> QueryResults<R> flatMap(
        ...
        qr[0] = new QueryResults<>(new FlatMapperIterator<>(iterator, i -> {
             ...
             */
            for (Query subQuery : results.queries()) {
                if (subQuery instanceof IdQuery) {
                    // 2023-03-23
                    // 释放IdQuery，ids过多导致内存OOM
                    qr[0].addQuery(new IdQuery(subQuery.resultType()));
                } else {
                    qr[0].addQuery(subQuery);
                }
            }
    ...
    @SuppressWarnings("unchecked")
    public static <T> CIter<T> emptyIterator() {
        return (CIter<T>) EMPTY_ITERATOR;
    }
