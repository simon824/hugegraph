...
public class BackendMutation {
    ...
    public void add(BackendEntry entry, Action action) {
        Id id = entry.id();
        assert id != null;
        List<BackendAction> backendActions = this.updates.get(entry.type(), id);
        if (backendActions != null) {
            backendActions.add(BackendAction.of(action, entry));
        } else {
    ...
    public void addWithOptimize(BackendEntry entry, Action action) {
        Id id = entry.id();
        assert id != null;
        List<BackendAction> backendActions = this.updates.get(entry.type(), id);
        if (backendActions != null) {
            this.update(entry, action, backendActions);
        } else {
            // If there is no entity with this id, add it
            this.updates.put(entry.type(), id, BackendAction.of(action, entry));
        }
    ...
    private void update(BackendEntry entry, Action action, List<BackendAction> items) {
        // assert items != null;
                ...
                case APPEND:
                    if (entry.type().isUniqueIndex() &&
                        ...
                        throw new IllegalArgumentException(String.format(
                                ...
                                " transaction between %s and %s", entry, originItem.entry()));
                    }
                case ELIMINATE:
                    if (originAction == Action.INSERT ||
    ...
    public Collection<Map<Id, List<BackendAction>>> actions() {
        return this.updates.actions();
    }
