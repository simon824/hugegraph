...
public class HugeEdge extends HugeElement implements Edge, Cloneable {
    ...
    public static HugeEdge constructEdge(HugeVertex ownerVertex,
        ...
        // boolean validLink = isOutEdge ?
        //                    edgeLabel.links().contains(Pair.of(ownerLabelId,
        //                                                       otherLabelId)) :
        //                    edgeLabel.links().contains(Pair.of(otherLabelId,
        //                                                       ownerLabelId));
        // if (!validLink) {
        //    throw new HugeException(ErrorCode.EDGE_INVALID_LINK, ownerLabelId,
        //                            otherLabelId, edgeLabel.name());
        // }

        VertexLabel srcLabel;
        VertexLabel tgtLabel;
        if (edgeLabel.edgeLabelType().general()) {
            // General类型的边 反序列的时候不指定两端顶点的vertexLabel,
            // 需要时会从后端查询得到顶点的label和properties信息
            srcLabel = new VertexLabel(graph, IdGenerator.of(0), VertexLabel.GENERAL_VL);
            tgtLabel = new VertexLabel(graph, IdGenerator.of(0), VertexLabel.GENERAL_VL);
        } else {
            // srcLabel = new VertexLabel(graph, IdGenerator.of(0),VertexLabel.GENERAL_VL);
            // tgtLabel = new VertexLabel(graph, IdGenerator.of(0),VertexLabel.GENERAL_VL);
            srcLabel = graph.vertexLabelOrNone(edgeLabel.sourceLabel());
            tgtLabel = graph.vertexLabelOrNone(edgeLabel.targetLabel());
        }
    ...
    public void assignId() {
        Id ownerVertexLabel = this.direction() == Directions.OUT ?
                              this.sourceVertex.schemaLabel().id() :
                              this.targetVertex.schemaLabel().id();
        Id otherVertexLabel = this.direction() == Directions.OUT ?
                              this.targetVertex.schemaLabel().id() :
                              this.sourceVertex.schemaLabel().id();
    ...
    protected boolean ensureFilledProperties(boolean throwIfNotExist) {
        ...
        Edge edge = this.graph().edge(this.id());
//        Edge edge = QueryResults.one(edges);
    ...
    public void vertices(HugeVertex owner, HugeVertex other) {
        ...
        if (this.label.edgeLabelType().general()) {
            // 如果边为General类型，当成out边添加入库，不进行矫正
            this.vertices(true, owner, other);
        } else {
            // 其他边类型根据label的links的指向关系进行边方向的矫正
    ...
    private void checkAdjacentVertexExist(HugeVertex vertex) {
        if (vertex.schemaLabel().undefined() &&
            this.graph() != null && /* when calculation sinking */
