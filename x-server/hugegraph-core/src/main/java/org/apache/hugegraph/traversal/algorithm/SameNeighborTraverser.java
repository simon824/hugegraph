...
public class SameNeighborTraverser extends HugeTraverser {
    ...
    public Set<Id> sameNeighbors(Id vertex, Id other, Directions direction,
        ...
        if (label != null) {
            return sameNeighbors(vertex, other, direction,
                                 ImmutableList.of(label), degree, limit);
        } else {
            return sameNeighbors(vertex, other, direction,
                                 ImmutableList.of(), degree, limit);
        }
    }
    ...
    public Set<Id> sameNeighbors(Id vertex, Id other, Directions direction,
        ...
        this.checkVertexExist(other, "other");
        ...
        Set<Id> sourceNeighbors = IteratorUtils.set(this.adjacentVertices(
                vertex, direction, labelsId, degree));
        Set<Id> targetNeighbors = IteratorUtils.set(this.adjacentVertices(
                other, direction, labelsId, degree));
        ...
        this.vertexIterCounter.addAndGet(2);
        if (limit != NO_LIMIT) {
    ...
    public Set<Id> sameNeighbors(List<Id> vertexIds, Directions direction,
                                 List<String> labels, long degree, long limit) {
        E.checkNotNull(vertexIds, "vertex id");
        E.checkArgument(vertexIds.size() >= 2,
