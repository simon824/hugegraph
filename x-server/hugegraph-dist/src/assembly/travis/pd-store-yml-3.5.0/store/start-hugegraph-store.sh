...
if [ "$JAVA_OPTIONS" = "" ]; then
    ...
    if [ $? -ne 0 ]; then
     ...
     JAVA_OPTIONS="-Xms${MIN_MEM}m -Xmx${XMX}m -XX:MetaspaceSize=256M  -XX:+UseG1GC  -XX:+UnlockExperimentalVMOptions  -XX:G1NewSizePercent=50  -XX:+ParallelRefProcEnabled   -XX:InitiatingHeapOccupancyPercent=80 -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=${LOGS} ${USER_OPTION} "
...
exec ${JAVA} ${JVM_OPTIONS} ${JAVA_OPTIONS} -jar -Dspring.config.location=${CONF}/application.yml \
    ${LIB}/hugegraph-store-3.5.0-SNAPSHOT.jar >> ${OUTPUT} 2>&1 &
