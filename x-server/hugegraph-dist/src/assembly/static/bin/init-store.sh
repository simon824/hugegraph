...
#### NOTE: this file is no longer need in hugegraph-1.5+, just start the server directly
...
# TODO: why use "-Djava.ext.dir"? How could it run well?
$JAVA -cp "$CP" org.apache.hugegraph.cmd.InitStore \
    ${CONF}/rest-server.properties ${GRAPH_SPACE} ${SERVICE_ID} ${NODE_ID} ${NODE_ROLE} \
    ${META_SERVERS} ${CLUSTER} ${PD_PEERS} ${WITH_CA} ${CA_FILE} ${CLIENT_CA} ${CLIENT_KEY}
